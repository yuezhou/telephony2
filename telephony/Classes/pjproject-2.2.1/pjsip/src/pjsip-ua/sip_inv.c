    ...
    if (invite_tsx && invite_tsx->status_code < 200) {
		...
		if(pjsua_var)
		{
			...
			if (!call) {
				...
				s1 = pj_str(rdata->msg_info.msg_buf);
				s2 = pj_str("Call completed elsewhere");
				if(pj_stristr(&s1, &s2))
				{
					PJ_LOG(4,(inv->obj_name, "CANCEL: Call answered elsewhere "));
					call->answeredElsewhere = 1;
				}
				else {
					call->answeredElsewhere = 0;
				}
				pj_mutex_unlock(pjsua_var->mutex);
			}
		}
